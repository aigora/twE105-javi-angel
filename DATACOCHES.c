...
void CrearEmpleado();
void CrearCliente();
void CrearCita(int x, char *nom_us);
void CrearVehiculo();
void MenuEmpleado(char nom_us[]);
void MenuCliente(char *nom_us);
void ConsultarDatosVehiculo();
void ConsultarCita(char *nom_us);
			...
			do{
        	    ...
        	    fflush(stdin);
   		    	if(strcmp(emp[i].contrasena, cont) == 0){
	        		aux = 3;
	   			    opcion1 = 1;
	            	nom_us = (char*)malloc((strlen(emp[i].nombre))*sizeof(char));//Memoria dinámica para guardar el nombre de usuario
	            	if (nom_us == NULL) { 
					    printf("Error: memoria no disponible.\n");
					    exit(-1); 
				    }
	            	strcpy(nom_us, emp[i].nombre);
    	    		printf("\n¡BIENVENIDO!\n\n");
	        	}
	        	else{
	        		Vaciar(cont);
			...
			do{
        	    ...
        	    fflush(stdin);
   		    	if(strcmp(clie[j].contrasena, cont) == 0){
	        		aux = 3;
	   			    opcion1 = 2;
	            	nom_us = (char*)malloc((strlen(clie[i].nombre))*sizeof(char));
					if (nom_us == NULL) { 
					    printf("Error: memoria no disponible.\n");
					    exit(-1); 
				    }
	            	strcpy(nom_us, clie[i].nombre);
    	    		printf("\n¡BIENVENIDO!\n\n");
	        	}
	        	else{
	        		Vaciar(cont);
...
void ConsultarCita(char *nom_us){
	...
	for(i = 0; i < n && aux == 0; i++){
		if(strcmp(*nom_us, cita[i].nombre) == 0){
			aux = 1;
		}
	}
	...
	if(aux == 1){
        ...
        printf("Si lo desea puede concertar otra cita(Si = s / No = n):");
	}
	else{
	...
	do{
		...
		if(res == 's' || res == 'S'){
			...
			printf("Si en algún otro momento desea concertar una cita, puede hacerlo a través del programa, o bien, llamando al 911 111 111");
		}
	}
